...
import { moduleNum, lessonNum, test } from './testData'
...
function checkAnswers() {
    ...
    const correctAnswer = test.value.answers[test.value.correctAnswer]
...
const prevTask = computed(() => {
    return `/tests/${moduleNum.value}/${lessonNum.value}/${+test.value.test - 1}`
})
...
<template>
    <section>
        ...
        <div class="container_course">
            ...
            <div class="ans">
                <button class="but" @click="onAnswerClick($event)" v-for="ans in test.answers">{{ ans }}</button>
            </div>
